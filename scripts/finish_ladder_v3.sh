#!/usr/bin/env bash
# Finish the ladder, take 3. Per user decision 07-21: run 13b cur_rb8 on GPU 1
# SINGLE (0-3 constraint kept). GPU 1 is free; GPU 0 has the user's own RAG run.
# 13b history: fine+shard->hang, cur+single(gpu0)->C-arm OOM, cur+shard(0,1)->
# calib OOM (GPU0 crowded by user's RAG). Single free GPU 1 avoids the contention;
# the remaining risk is the single-GPU C-arm OOM (2 model copies) — monitored.
# Plan (GPUs 0-3 only):
#   13b       cur_rb8  single GPU 1  (now)
#   deepseek  fine_rb8 shard 2,3     (running, pid 41307)
#   openllama fine_rb8 shard 2,3     (after deepseek)
...
_run () {  # cfgname devices devmap logtag model
  ...
  if [ -f "$OUT" ] && grep -q "^${model}," "$OUT"; then
    echo "[v3] === $lg SKIP (already in $OUT) $(date +%m-%d_%H:%M)"; return 0
  fi
  echo "[v3] >>> $lg ($cfgname) START dev=$dev dm=$dm $(date +%m-%d_%H:%M)"
  CUDA_VISIBLE_DEVICES=$dev CAREKV_DEVICE_MAP=$dm CAREKV_CFG_NAME=$cfgname \
    ...
    --append-csv "$OUT" > "$RD/v3_${lg}.log" 2>&1
  echo "[v3] >>> $lg END rc=$? $(date +%m-%d_%H:%M) :: $(grep 'DONE' "$RD/v3_${lg}.log" | grep -v '^  BlockGTQ:' | tail -1)"
}
...
waitpid () { local pid=$1 tag=$2; while kill -0 "$pid" 2>/dev/null; do sleep 60; done
             echo "[v3] $tag (pid $pid) exited $(date +%m-%d_%H:%M)"; }

# 1) 13b cur_rb8 on single free GPU 1 (no device_map).
run_cur 1 "" 13b NousResearch/Llama-2-13b-hf &
PID_13B=$!

# 2) openllama fine_rb8 on 2,3 after deepseek frees them.
echo "[v3] waiting on deepseek (pid $PID_DEEPSEEK) to free GPU 2,3 $(date +%m-%d_%H:%M)"
...
wait "$PID_13B"
echo "[v3] 13b + openllama done — starting Yi-34B (cur_rb8, 4-GPU) $(date +%m-%d_%H:%M)"
run_cur 0,1,2,3 auto Yi-34B 01-ai/Yi-34B
echo "[v3] ALL DONE $(date +%m-%d_%H:%M)"
