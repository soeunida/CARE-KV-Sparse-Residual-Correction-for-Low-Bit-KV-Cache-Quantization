#!/usr/bin/env bash
set -euo pipefail

cd /home/soeun/CARE_KV/care_kv
mkdir -p results

export CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0}
export MODEL_ID=${MODEL_ID:-TinyLlama/TinyLlama-1.1B-Chat-v1.0}
export PYTHONPATH=${PYTHONPATH:-/home/soeun}

SEQ_LIST="${SEQ_LIST:-128}"
BITS_LIST="${BITS_LIST:-3 2}"
R_LIST="${R_LIST:-0.001 0.002 0.005 0.01 0.02 0.03 0.05 0.10}"

echo "MODEL_ID=$MODEL_ID"
echo "CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES"
echo "SEQ_LIST=$SEQ_LIST"
echo "BITS_LIST=$BITS_LIST"
echo "R_LIST=$R_LIST"

for L in $SEQ_LIST; do
  echo "===== FP16 SEQ_LEN=$L ====="
  MODE=fp16 \
  SEQ_LEN=$L \
  python eval_ppl_simple.py | tee results/ppl_fp16_${L}_sweep.txt

  for B in $BITS_LIST; do
    echo "===== BASE_QUANT INT$B SEQ_LEN=$L ====="
    MODE=carekv \
    CAREKV_RETURN=care \
    CAREKV_PREFILL_MODE=base_quant \
    BASE_BITS=$B \
    SEQ_LEN=$L \
    python eval_ppl_simple.py | tee results/ppl_basequant_int${B}_${L}_sweep.txt

    for R in $R_LIST; do
      echo "===== CAREKV INT$B SEQ_LEN=$L V-only R=$R ====="
      MODE=carekv \
      CAREKV_RETURN=care \
      CAREKV_PREFILL_MODE=carekv \
      CAREKV_PREFILL_RESIDUAL_KIND=v \
      BASE_BITS=$B \
      CAREKV_PREFILL_RESIDUAL_RATIO=$R \
      CAREKV_K_CORRECTION_SCALE=0.0 \
      SEQ_LEN=$L \
      python eval_ppl_simple.py | tee results/ppl_carekv_int${B}_${L}_Vonly_R${R}_sweep.txt
    done
  done
done

echo "===== SUMMARY ====="
python tools/summarize_ppl_results.py results/ppl_*_sweep.txt
