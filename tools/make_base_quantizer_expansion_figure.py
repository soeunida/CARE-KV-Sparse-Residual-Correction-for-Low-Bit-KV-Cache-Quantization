"""Two figures for the base-quantizer-expansion comparison:
   - fig_base_quantizer_expansion_ppl.png             (PPL bar chart)
   - fig_base_quantizer_expansion_memory_quality.png  (scatter, log-y)
"""
...
FAMILY_COLORS = {
    ...
    "kvquant_plus_carekv":   "#7f7f7f",
    "rotatekv_style":        "#17becf",
    "rotatekv_plus_carekv":  "#e377c2",
    "turboquant_style":      "#d62728",
    "turboquant_plus_carekv":"#8c564b",
}
...
def fig_ppl(rows, out, label):
    ...
    ax.set_title(f"Base-quantizer expansion — {label}", fontsize=12)
    ax.grid(axis="y", linestyle="--", alpha=0.4)
    fig.text(0.5, 0.002,
             "KVQuant/RotateKV/TurboQuant are same-condition reimpls (NOT official); "
             "unsupported cells (TurboQuant + KVQuant+CARE-KV) omitted from bar chart.",
...
def fig_memory_quality(rows, out, label):
    ...
    for r in rows:
        ...
        size = 180 if fam in ("fp16",) else (140 if "carekv" in fam else 80)
