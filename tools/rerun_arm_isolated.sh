...
# Args: MODEL_ID GPUS OUT_CSV [SEQ_LEN=512] [NUM_SAMPLES=4]
set -u
MODEL_ID="$1"; GPUS="$2"; OUT="$3"; SL="${4:-512}"; NS="${5:-4}"
...
rm -f "$OUT"
echo "[rerun] $(date '+%T') $MODEL_ID on GPU=$GPUS dmap=${CAREKV_DEVICE_MAP:-single} SL=$SL N=$NS"
for arm in fp16 base_int3 carekv; do
  echo "[rerun] --- arm=$arm ---"
  python tools/eval_7b_validation.py --model-id "$MODEL_ID" --out-csv "$OUT" \
    --seq-len "$SL" --num-samples "$NS" --arm "$arm"
done
